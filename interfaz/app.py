...
class AsistenteApp:
    ...
    def crear_widgets(self):
        ...
        self.mtr_salida  = _metric(inner_m, "🔊", "Salida Audio", "---",         TEXT_MUTED)
        self.mtr_mic     = _metric(inner_m, "◎", "Mic",        "---",            TEXT_MUTED)
        self.mtr_oled    = _metric(inner_m, "◈", "OLED",       "---",            TEXT_MUTED)
                    ...
                    while _t.time() - t0 < 8:
                        if esp32_comm.serial_conn.in_waiting > 0:
                            ...
                            if "OLED_TEST_OK" in linea or "OLED_ANIM_OK" in linea:
                                ...
                                if hasattr(self, "mtr_oled"):
                                    self.root.after(0, lambda: self.mtr_oled.configure(text=" OK", fg=CLR_GREEN))
                                return
                        _t.sleep(0.05)
    ...
    def probar_audio_real(self):
        def _run():
            ...
            if exito and "AUDIO_TEST_OK" in res:
                ...
                if hasattr(self, "mtr_salida"):
                    self.root.after(0, lambda: self.mtr_salida.configure(text=" OK", fg=CLR_GREEN))
                ...
                if hasattr(self, "mtr_salida"):
                    self.root.after(0, lambda: self.mtr_salida.configure(text=" FALLO", fg=CLR_RED))
        threading.Thread(target=_run, daemon=True).start()
                ...
                for chunk in r_stream.iter_content(chunk_size=4096):
                    ...
                    if a != -1 and b != -1:
                        ...
                        if img is not None:
                            ...
                            def _update_stream_gui(fps_val=fps, width=w, height=h, size=len(jpg)/1024.0, img_array=img_rgb):
                                img_pil = Image.fromarray(img_array).resize((400, 266), Image.Resampling.LANCZOS)
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    if img is not None:
                        ...
                        def _update_gui(fps_val=fps, lat=t_trans, width=w, height=h, size=size_kb, img_array=img_rgb):
                            img_pil = Image.fromarray(img_array).resize((400, 266), Image.Resampling.LANCZOS)
                            self.cam_img_tk = ImageTk.PhotoImage(img_pil)
                            if getattr(self, 'canvas_cam_img_id', None) is None:
                                self.canvas_cam.delete("all")
        ...
        if self.stream_activo:
            ...
            while self.stream_activo:
                ...
                if img is not None:
                    ...
                    def _update_serial_gui(fps_val=fps, width=w, height=h, img_array=img_rgb):
                        img_pil = Image.fromarray(img_array).resize((400, 266), Image.Resampling.LANCZOS)
                        self.cam_img_tk = ImageTk.PhotoImage(img_pil)
                        if getattr(self, 'canvas_cam_img_id', None) is None:
                            self.canvas_cam.delete("all")
    ...
    def escanear_cripto_pipeline(self):
        def _run():
            ...
            if cripto:
                ...
                self.root.after(0, _update_ui_img)
    ...
    def consultar_voz_mic_fisico(self):
        def _run():
            self.agregar_mensaje("Usuario", "🎙️ [🔴 ESCUCHANDO — Habla ahora por el micrófono INMP441 del ESP32 por 4 segundos...]", "user")
            esp32_comm.enviar_comando_oled("ESCUCHANDO")
            
            # Captura silenciosa desde INMP441 sin reproducción (nuevo comando MIC_CAPTURE)
            pcm_bytes, estado = esp32_comm.grabar_voz_para_stt(4)
            esp32_comm.enviar_comando_oled("PROCESANDO")
            
            if pcm_bytes:
                texto = escuchar_desde_pcm(pcm_bytes)
            else:
                texto = f"No se recibió audio del INMP441: {estado}"
            
            self.agregar_log_consola(f"[STT INMP441] Transcripción: '{texto}'")
            self.agregar_mensaje("Usuario", f"🎙️ \"{texto}\"", "user")
            
            if texto and texto != "No se reconoció el comando de voz." and not texto.startswith("Error") and not texto.startswith("No se recibió"):
