...
def obtener_info_cripto(nombre):
    ...
    try:
        ...
        try:
            ...
            # Construir ticker de yfinance con el nombre real de la cripto, no hardcodear BTC
            ticker_yf = f"{nombre_lower.upper()}-USD"
            t = yf.Ticker(ticker_yf)
            fast_info = t.fast_info
            last_price = getattr(fast_info, 'last_price', None) or getattr(fast_info, 'regularMarketPrice', None)
            if last_price and float(last_price) > 0:
                return {
                    "nombre": nombre.capitalize(),
                    "simbolo": nombre_lower.upper(),
                    "precio_usd": float(last_price),
                    "precio_mxn": float(last_price) * 20.2,
                    "market_cap": getattr(fast_info, 'market_cap', 0) or 0,
                    "volumen_24h": getattr(fast_info, 'three_month_average_volume', 0) or 0,
                    "cambio_24h": 0.0,
                    "ranking": 0,
                    "descripcion": "Datos obtenidos vía Yahoo Finance (Fallback).",
                    "imagen": ""
                }
            return None  # yfinance tampoco encontró datos reales
