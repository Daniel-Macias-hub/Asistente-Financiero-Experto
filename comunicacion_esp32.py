...
class ComunicacionESP32:
    def __init__(self, puerto="COM11", baudrate=921600):
        self.puerto = puerto
    ...
    def detener_operacion(self):
        ...
        # Intentamos obtener el lock con un timeout muy corto para no bloquear la GUI
    ...
    def conectar(self, puerto=None):
        ...
        with self.lock:
            try:
                ...
                # 1. Abrir puerto con timeout adecuado (1.5s lectura, 5s escritura)
                self.serial_conn = serial.Serial(self.puerto, self.baudrate, timeout=1.5, write_timeout=5)
    ...
    def enviar_comando_oled(self, estado: str, texto_extra: str = ""):
        ...
        # Timeout muy corto para evitar que el hilo principal (Tkinter) se bloquee
        # si hay una transmisión de audio pesada en curso (que retiene el lock)
    ...
    def reproducir_audio_bocina_pcm(self, pcm_bytes: bytes):
        """Transmite bytes PCM codificados en Base64 al ESP32-S3 con control de flujo ACK."""
        if not self.conectado or not self.serial_conn or not pcm_bytes:
            print("[AUDIO TX] Abortado: ESP32 no conectado o sin audio")
        ...
        with self.lock:
            try:
                ...
                print(f"[AUDIO TX] Enviando comando: {cmd_str.strip()}")
                ...
                if not ready:
                    print("[AUDIO TX] ERROR: TIMEOUT esperando AUDIO_PLAY_READY")
                    return False, "TIMEOUT AUDIO_PLAY_READY"

                print(f"[AUDIO TX] READY recibido. Enviando {len(pcm_bytes)} bytes en bloques de 512...")
                import base64
                chunk_size = 512
                ...
                for idx in range(0, len(pcm_bytes), chunk_size):
                    ...
                    # Eliminamos self.serial_conn.flush() aquí. 
                    # En Windows, flush puede bloquear indefinidamente si el ESP32 no procesa a tiempo.
                    
                    # Esperar ACK "K" del ESP32 antes de enviar el siguiente bloque
                    t_ack = time.time()
                    ack_ok = False
                    while time.time() - t_ack < 3.0:
                        if self.cancelar_flag:
                            self.serial_conn.write(b"STOP\n")
                            return False, "CANCELADO"
                        if self.serial_conn.in_waiting > 0:
                    ...
                    if not ack_ok:
                        print(f"[AUDIO TX] ERROR: Timeout ACK en bloque {chunks_ok}/{total_chunks}")
                        return False, f"TIMEOUT ACK bloque {chunks_ok}"
                    
                    chunks_ok += 1
                    if chunks_ok % 50 == 0:
                        print(f"[AUDIO TX] Progreso: {chunks_ok}/{total_chunks} bloques enviados")

                self.serial_conn.write(b"AUDIO_PLAY_END\n")
                self.serial_conn.flush()
                print(f"[AUDIO TX] Todos los {chunks_ok} bloques enviados. Esperando AUDIO_PLAY_OK...")

                # Esperar AUDIO_PLAY_OK
                ...
                while time.time() - t_ok < timeout_ok:
                    ...
                    if self.serial_conn.in_waiting > 0:
                        ...
                        if "AUDIO_PLAY_OK" in linea:
                            print(f"[AUDIO TX] AUDIO_PLAY_OK recibido. Reproducción completada.")
                            break
                    time.sleep(0.05)

                return True, "AUDIO_PLAY_OK"
            except Exception as e:
                print(f"[AUDIO TX] EXCEPCION: {e}")
                return False, str(e)
    ...
    def grabar_voz_para_stt(self, duracion_sec=4):
        """
        Graba audio silenciosamente desde el INMP441 y devuelve los bytes PCM
        para transcripción por Vosk. NO reproduce el audio (a diferencia de MIC_TEST).
        Usa el comando MIC_CAPTURE:<seg> del firmware.
        """
        if not self.conectado or not self.serial_conn:
            return None, "ESP32 no conectado"
        ...
        with self.lock:
            try:
                ...
                self.serial_conn.reset_input_buffer()
                cmd = f"MIC_CAPTURE:{duracion_sec}\n".encode('utf-8')
                self.serial_conn.write(cmd)
                self.serial_conn.flush()

                # Esperar MIC_CAPTURE_START (hasta dur + 2s de margen)
                timeout = duracion_sec + 6
                t0 = time.time()
                started = False
                while time.time() - t0 < timeout:
                    ...
                    if self.serial_conn.in_waiting > 0:
                        ...
                        if "MIC_CAPTURE_ERROR" in line:
                            return None, line
                        if "MIC_CAPTURE_START" in line:
                            started = True
                            break
                    time.sleep(0.05)

                if not started:
                    return None, "TIMEOUT MIC_CAPTURE_START"

                # Recolectar bloques Base64 hasta MIC_CAPTURE_END
                pcm_chunks = []
                t1 = time.time()
                while time.time() - t1 < 30:
                    ...
                    if self.serial_conn.in_waiting > 0:
                        line = self.serial_conn.readline().decode('utf-8', errors='ignore').strip()
                        if line == "MIC_CAPTURE_END":
                            break
                        if line:
                            try:
                                pcm_chunks.append(_b64.b64decode(line))
                            except Exception:
                                pass
                    else:
                        time.sleep(0.01)
                ...
                if not pcm_chunks:
                    return None, "Sin datos PCM recibidos"
